    ...
    <main suppressHydrationWarning className="w-full px-2 py-4">
      {!isLoading ? (
        <div className="grid grid-cols-3 gap-4 w-full">
          {[1, 2, 3, 4].map((_, indx) => (
            <div key={indx} className="rounded-lg border bg-input/20 border-input p-2 flex w-full col-span-3">
              <Skeleton
                className={cn(
          ...
          <div className="grid col-span-2 gap-4 lg:grid-cols-3 grid-cols-1">
            <div className="p-5 flex flex-col items-center gap-4 border border-input text-card-foreground shadow rounded-2xl w-full col-span-2">
            ...
            <div className="grid w-full gap-4 grid-cols-1 ">
              <div className="p-6 shadow-lg flex flex-col justify-center items-center gap-6 border border-input rounded-2xl w-full overflow-y-auto">
                <Skeleton className="w-72 h-3.5" />
                <div className="grid grid-cols-8 gap-4 w-full">
                  {new Array(48).fill(null).map((_, indx) => (
                    <Skeleton
                      key={indx}
                      className="h-8 lg:h-6 w-full rounded-md mt-2"
                    />
                  ))}
                </div>
              </div>
              <div className="p-5 flex flex-col items-center gap-7 border border-input shadow-lg rounded-2xl w-full overflow-y-auto">
